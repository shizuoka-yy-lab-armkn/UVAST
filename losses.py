...
class FrameWiseLoss(nn.Module):
    ...
    def __call__(self, predictions, batch_target, mask, epoch):
                ...
                for fcls in torch.unique(batch_target).view(-1):
                    if fcls != -1:
                        ...
                        if self.args.framewise_loss_g_apply_logsoftmax:
                            ...
                            assert (
                                self.args.framewise_loss_g_apply_nothing or self.args.framewise_loss_g_apply_logsoftmax
                            ), " you need to define the framewise group model"
                        # computing the average
                        suming_similar_act = (
                            (predictions_group * ((batch_target == fcls) + 0.0).unsqueeze(1))
                            .sum(2)
                            .sum(0, keepdim=True)
                        )
        ...
        self.update_metrics(
            ...
            framewise_loss,
            framewise_loss_g,
            framewise_loss_random_singleframe,
        )
        return loss
...
class SegmentLossAction(nn.Module):
    ...
    def __call__(self, pred_action, target_action, vid_len, epoch):
            ...
            for fcls in torch.unique(target_action).view(-1):
                if fcls != -1:
                    ...
                    for i in range(len(pred_action)):  # this can be also in a outside loop
                        ...
                        # computing the average
                        suming_similar_act = (
                            (predictions_group * ((target_action == fcls) + 0.0).unsqueeze(1))
                            .sum(2)
                            .sum(0, keepdim=True)
                        )
        ...
        self.update_metrics(
            pred_action[-1], target_action, g_pred, g_gt, loss, segwise_loss, segwise_loss_g, segwise_loss_g_indecs
        )
        return loss

    def update_metrics(
        self, pred_action, target_action, g_pred, g_gt, loss, segwise_loss, segwise_loss_g, segwise_loss_g_indecs
    ):
        # update losses
    ...
    def log_metrics(self, mode, epoch, writer=None):
        print(
            f"[{mode} epoch {epoch}] Loss: {self.losses.avg:.4f}, "
            ...
            f"Acc Segment-W: {self.acc_action.avg:.4f}, " f"Acc Group Segment-W: {self.acc_g.avg:.4f}",
            flush=True,
        )
...
class AttentionLoss(nn.Module):
    ...
    def __call__(self, pred_attn, target_attn: torch.Tensor, batch_target: torch.Tensor):
        ...
        if self.args.do_crossattention_action_loss_nll:
            for i in range(len(pred_attn)):
                for batchnum in range(pred_attn[i].shape[0]):
                    valid_pred_attn_lln = (
                        pred_attn[i]
                        .permute(0, 2, 1)[batchnum]
                        .unsqueeze(0)[batch_target[batchnum].repeat(self.args.n_head_dec, 1) != -1]
                    )
                    valid_target_attn_lln = (
                        (target_attn.permute(0, 2, 1)[batchnum][batch_target[batchnum] != -1])
                        .repeat(self.args.n_head_dec, 1)
                        .to(self.args.device)
                    )
                    valid_pred_attn_log = self.LogSoftmax(valid_pred_attn_lln / self.args.temperature)
                    attn_loss_tmp = attn_loss_tmp + (
                        self.NLLLoss(valid_pred_attn_log, valid_target_attn_lln.argmax(1).view(-1).long())
                        * float(1 / pred_attn[i].shape[0])
                    )
        ...
        self.update_metrics(
            ...
            pred_attn,
            target_attn,
            valid_pred_attn_lln_all,
            valid_target_attn_lln_all,
        )
        return loss

    def update_metrics(
        self, loss, attn_loss_nll, attn_loss_ce, pred_attn, target_attn, valid_pred_attn_lln, valid_target_attn_lln
    ):
        # update losses
        self.losses.update(loss.item())
    ...
    def log_metrics(self, mode, epoch, writer=None):
        print(
            ...
            f"Acc Attention Map: {self.acc_attn_lln.avg:.4f}",
            flush=True,
        )
...
class DurAttnCALoss(nn.Module):
    ...
    def update_metrics(
        self, loss, attn_loss_segmentid_ce, attn_loss_segmentid_ce_inside_CA, pred_attn_bmm, target_attn
    ):
        # update losses
        self.loss.update(loss.item())
    ...
    def log_metrics(self, mode, epoch, writer=None):
        print(
            ...
            f"Loss Attention CE inside dec: {self.attn_loss_segmentid_ce_inside_CA.avg:.4f}",
            flush=True,
        )
