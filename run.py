...
# general arguments
parser.add_argument(
    "--features_dim", default="2048", type=int, help="dimension of input features (I3D features are 2048 dim)"
)
parser.add_argument(
    "--experiment_path", default="/path_to_directory/", type=str, help="directory to save experiment results"
)
...
parser.add_argument(
    "--data_root_mean_duration",
    default="/path_to_data_directory/",
    type=str,
    help="root to the mean durations of the datasets",
)
...
parser.add_argument(
    "--num_classes", default=-1, type=int, help="number of classes for each dataset (gtea=11, salads=19, breakfast=48)"
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="optional augmention for training: randomly dropping frames",
)
parser.add_argument("--split_segments", default=False, action="store_true", help="splitting segments")
parser.add_argument(
    "--split_segments_max_dur",
    default="0.1",
    type=float,
    help="max duration in split_segments; for details see the paper",
)
parser.add_argument("--use_cuda", action="store_true", help="use cuda for training")
parser.add_argument(
    "--exp_name",
    default="choose_A_NAME",
    type=str,
    help="name of the experiment, specifies the folder where models are saved",
)
parser.add_argument("--seed", default=None, type=int, help="specify seed number")
parser.add_argument(
    "--save_args", default=False, action="store_true", help="save arguments that were used for training"
)
# testing
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="initialize durations in fifa with alignment decoder predictions",
)
...
parser.add_argument(
    "--do_refine_actions_no_consecutive_similar_action",
    action="store_true",
    default=False,
    help="remove duplicate actions during viterbi inference",
)
# encoder arguments
...
parser.add_argument(
    "--encoder_model",
    default="asformer_advanced",
    type=str,
    choices=["asformer_advanced", "asformer_org_enc", "asformer_org_enc_dec"],
    help="select encoder model",
)
parser.add_argument(
    "--enc_norm_type", default="InstanceNorm1d", type=str, help=["LayerNorm", "InstanceNorm1d_track", "InstanceNorm1d"]
)
parser.add_argument("--num_layers_enc", default=10, type=int, help="the number of encoder layers in ASFormer")
parser.add_argument(
    "--num_layers_asformer_dec",
    default=10,
    type=int,
    help="number of decoder layers in the ASFormer (only used with asformer_org_enc_dec)",
)
# transcript decoder arguments
parser.add_argument(
    "--n_head_dec", default=1, type=int, help="the number of heads of the transcript decoder (first stage)"
)
parser.add_argument(
    "--AttentionPoolType_dec",
    default="none",
    type=str,
    help="option to smooth the cross attention; options are: none, avg, max",
)
parser.add_argument(
    "--AttentionPoolKernel_dec",
    default=31,
    type=int,
    help="kernel size for smoothing if --AttentionPoolType_dec is selected",
)
parser.add_argument(
    "--num_layers_dec", default=2, type=int, help="number of layers of the transcript decoder (first stage)"
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="use positional encoding for target in the transcript decoder",
)
parser.add_argument(
    "--use_pe_src",
    default=False,
    action="store_true",
    help="use positional encoding for source in the transcript decoder",
)
parser.add_argument(
    "--dec_dim_feedforward",
    default=2048,
    type=int,
    help="similar to dec_dim_feedforward option in the standard pytorch model",
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="use alignment decoder for duration prediction (second stage)",
)
...
parser.add_argument(
    "--AttentionPoolType_dec_dur",
    default="none",
    type=str,
    help="option to smooth the cross attention; options are: none, avg, max",
)
parser.add_argument(
    "--AttentionPoolKernel_dec_dur",
    default=0,
    type=int,
    help="kernel size for smoothing if --AttentionPoolType_dec_dur is selected",
)
parser.add_argument("--dropout_dec_dur", default=0.1, type=float, help="dropout for the alignment decoder")
parser.add_argument(
    "--dec_dim_feedforward_dec_dur",
    default=1024,
    type=int,
    help="similar to dec_dim_feedforward option in the standard pytorch model",
)
...
parser.add_argument(
    "--alignment_decoder_model",
    default="uvast_decoder",
    type=str,
    choices=["uvast_decoder", "pytorch_decoder"],
    help="select alignment decoder model",
)
parser.add_argument("--pretrained_model", default=None, type=str, help="path to pretrained model from first stage")
# optimizer arguments
parser.add_argument("--bs", default="1", type=int, help="batch size")
parser.add_argument(
    "--usebatch", default=False, action="store_true", help="if bs > 1 is used the samples need to be padded"
)
...
parser.add_argument(
    ...
    default=10,
    type=int,
    help="adaptive gradient clipping is based on percentile of the previous gradient",
)
# loss arguments
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="use cross attention loss for first stage",
)
parser.add_argument(
    "--do_crossattention_dur_loss_ce",
    default=False,
    action="store_true",
    help="use cross attention loss for second stage",
)
parser.add_argument(
    "--framewise_loss_g_apply_logsoftmax",
    default=False,
    action="store_true",
    help="type of the normalization for group wise CE",
)
parser.add_argument(
...
parser.add_argument("--save_frames_to_segment_assignment", default=False, action="store_true")
...
elif args.dataset == "bike_master_botsu_0614_full":
    args.num_classes = 37
    args.sample_rate = 1
    args.channel_masking_rate = 0.4
...
elif args.dataset == "bikeframe_students_live_blip2":
    args.num_classes = 36
    args.sample_rate = 1
    args.channel_masking_rate = 0.4
    args.split_segments_max_dur = 0.15
...
if args.exp_name:
    ...
    print(
        "Oops!! You did not provide exp_name and it can cause your experiments results be saved in one folder and cause issues later!"
    )
...
training_dataloader = DataGenerator(
    ...
    len_seg_max=args.len_seg_max,
    features_path=args.features_path,
    feature_type=args.feature_type,
    feature_mode=args.feature_mode,
)
trainloader = torch.utils.data.DataLoader(
    training_dataloader, batch_size=args.bs, shuffle=True, num_workers=4, pin_memory=True
)
# dataloaders testing
testing_dataloader = DataGenerator(
    data_root=args.data_root,
    split=args.split,
    dataset=args.dataset,
    mode="val",
...
testloader = torch.utils.data.DataLoader(
    testing_dataloader, batch_size=1, shuffle=False, num_workers=4, pin_memory=True
)
print("starting the training:")
trainer = Trainer(args)
trainer.train(
    args, device=device, trainloader=trainloader, testloader=testloader, testing_dataloader=testing_dataloader
)
